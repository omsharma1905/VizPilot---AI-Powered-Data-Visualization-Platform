/**
 * Grouped Bar Chart Builder — Phase 2D
 */
...
export function buildGroupedBarChart(
  ...
  if (!primaryName) {
    throw new VisualizationError(
      ...
      'Grouped bar chart requires a primary dimension field (x).',
      400
    );
  }
  ...
  if (!secondaryName || secondaryName === primaryName) {
    throw new VisualizationError(
      'VISUALIZATION_FIELD_NOT_FOUND',
      'Grouped bar chart requires a secondary dimension field (color or category).',
      400
    );
  }
  ...
  if (!yColName) {
    throw new VisualizationError(
      'VISUALIZATION_FIELD_NOT_FOUND',
      'Grouped bar chart requires at least one measure field (y[0]).',
      400
    );
  }
  ...
  if (renderedCategories.length > MAX_RENDER_POINTS) {
    context.warnings.push(
      `Grouped bar chart categories (${renderedCategories.length}) exceeded limit of ${MAX_RENDER_POINTS}. Truncating.`
    );
    renderedCategories = renderedCategories.slice(0, MAX_RENDER_POINTS);
  }

  const base = createBaseOption(`${valField.column.name} by ${primField.column.name} & ${secField.column.name}`);
